...
class mphb(object):
    def __init__(self, phase, rebuild=False):
        ...
        self.data_path = os.path.join(cfg.MPHB_PATH , '')
    ...
    def load_labels(self):
        cache_file = os.path.join(
            self.cache_path, 'mphb_' + self.phase + '_gt_labels.pkl')
        ...
        if self.phase == 'train':
            txtname = os.path.join(
                self.data_path, 'MPHB-label-txt', 'trainval.txt')
        else:
            txtname = os.path.join(
                self.data_path, 'MPHB-label-txt', 'test.txt')
        with open(txtname, 'r') as f:
            self.image_index = [x.strip() for x in f.readlines()]

        gt_labels = []
        for index in self.image_index:
            label, num = self.load_mphb_annotation(index)
            if num == 0:
                continue
